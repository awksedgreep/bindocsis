...
// Top-level names come from the PacketCable TLV table; PacketCable
// sub-TLVs have no spec table in bindocsis, so inside a block use
// TLV<n> with raw bytes (hex or a quoted string).
...
# Enable/disable network access (0=disabled, 1=enabled)
NetworkAccessControl Enabled

# PacketCable MTA configuration file container
MTAConfigurationFile {
    TLV1 73 69 70
    TLV2 01
}

# Kerberos realm for PacketCable security
KerberosRealm "PACKETCABLE.EXAMPLE.COM"

# DNS server address for the MTA
DNSServer 192.168.1.1
```
(Comment text is the spec description for each TLV; sub-TLVs of
PacketCable containers are written as `TLV<n>` with raw bytes.)
