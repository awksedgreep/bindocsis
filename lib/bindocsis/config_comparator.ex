defmodule Bindocsis.ConfigComparator do
  ...
  ## Example
      ...
      1
  """
  @spec compare(binary(), binary(), comparison_options()) ::
          {:ok, comparison_result()} | {:error, String.t()}
  def compare(config_a, config_b, opts \\ []) do
    ...
    with {:ok, analysis_a} <- ConfigAnalyzer.analyze(config_a, docsis_version: docsis_version),
         ...
         {:ok, parsed_a} <-
           Bindocsis.parse(config_a,
             format: :binary,
             enhanced: true,
             docsis_version: docsis_version
           ),
         {:ok, parsed_b} <-
           Bindocsis.parse(config_b,
             format: :binary,
             enhanced: true,
             docsis_version: docsis_version
           ) do
  ...
  def analyze_impact(%{impact_analysis: impact_analysis})
      when impact_analysis != %{analyzed: false} do
    {:ok, impact_analysis}
  end
  ...
  defp compare_summaries(analysis_a, analysis_b) do
    ...
    changes =
      if analysis_a.service_tier != analysis_b.service_tier do
        [
          %{
            category: :service_tier,
            change_type: :modified,
            description:
              "Service tier changed from #{analysis_a.service_tier} to #{analysis_b.service_tier}",
            impact_level: :high
          }
          | changes
        ]
    ...
    changes =
      if analysis_a.configuration_type != analysis_b.configuration_type do
        [
          %{
            category: :configuration_type,
            change_type: :modified,
            description:
              "Configuration type changed from #{analysis_a.configuration_type} to #{analysis_b.configuration_type}",
            impact_level: :high
          }
          | changes
        ]
  ...
  defp generate_change_statistics(tlv_changes) do
    ...
    %{
      ...
      change_percentage:
        if(total > 0, do: Float.round((total - unchanged) / total * 100, 1), else: 0.0)
    }
  end
  ...
  defp assess_performance_impact(analysis_a, analysis_b) do
    ...
    cond do
      ...
      complexity_b > complexity_a * 1.5 -> :positive
      # Significantly less complex
      complexity_b < complexity_a * 0.5 -> :negative
      true -> :neutral
    end
  end
  ...
  defp generate_change_recommendations(tlv_changes, overall_impact) do
    ...
    recommendations =
      case overall_impact do
        :critical ->
          [
            "CRITICAL: Thoroughly test configuration changes before deployment",
            "Consider staged rollout with rollback plan" | recommendations
          ]

        :high ->
          [
            "HIGH IMPACT: Test changes in lab environment first",
            "Monitor service closely after deployment" | recommendations
          ]

        :medium ->
          [
            "MEDIUM IMPACT: Review changes with network team",
            "Schedule deployment during maintenance window" | recommendations
          ]

        :low ->
          ["LOW IMPACT: Changes appear safe for deployment" | recommendations]
      end
    ...
    recommendations =
      if frequency_changes do
        [
          "Frequency changes detected - ensure new frequency is available and licensed"
          | recommendations
        ]
      else
        recommendations
  ...
  defp build_detailed_changes_section(tlv_changes) do
    if length(tlv_changes) == 0 do
      ...
      sections =
        if Map.has_key?(changes_by_type, :added) do
          added_section = """
          ### Added TLVs
      ...
      sections =
        if Map.has_key?(changes_by_type, :removed) do
          removed_section = """
          ### Removed TLVs
      ...
      sections =
        if Map.has_key?(changes_by_type, :modified) do
          modified_section = """
          ### Modified TLVs
