defmodule Bindocsis.Parsers.YamlParser do
  @moduledoc """
  Parser for YAML-formatted DOCSIS configurations.

  Converts YAML configuration data directly to TLV structures without
  ...
  @doc """
  Parses a YAML string and returns TLV structures.
  ...
  def parse(yaml_string) when is_binary(yaml_string) do
    case YamlElixir.read_from_string(yaml_string) do
      ...
      {:ok, _other} ->
        {:error, "YAML must contain a 'tlvs' array at the root level"}
