defmodule Bindocsis.Parsers.JsonParser do
  ...
  ```json
  {
    "tlvs": [
      {"type": 3, "value": 1},
      {"type": 24, "subtlvs": [{"type": 1, "value": 1}]}
    ]
  }
  ```
  """
  ...
  # Extract TLVs from parsed JSON data
  defp extract_tlvs(%{"tlvs" => tlvs}) when is_list(tlvs) do
    try do
      converted_tlvs = Enum.map(tlvs, &convert_json_tlv/1)
      {:ok, converted_tlvs}
    rescue
      error ->
  ...
  defp extract_tlvs(%{}) do
    {:error, "Missing 'tlvs' array in JSON"}
  end

  defp extract_tlvs(_) do
    {:error, "Invalid JSON structure: expected object with 'tlvs' array"}
  end
  ...
  defp convert_json_tlv(invalid_tlv) do
  ...
  defp convert_value_to_binary(value) when is_binary(value) do
    # Handle hex strings like "AA BB CC"
  ...
  # Check if string looks like hex (e.g., "AA BB CC" or "AABBCC")
  defp hex_string?(str) when is_binary(str) do
    # Remove spaces and check if all chars are hex
  ...
  def validate_structure(_) do
    {:error, "Invalid JSON structure: must contain 'tlvs' array"}
  end
