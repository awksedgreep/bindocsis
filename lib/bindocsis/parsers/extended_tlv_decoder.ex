defmodule Bindocsis.Parsers.ExtendedTlvDecoder do
  ...
  defp decode_tlvs(data, _acc) when byte_size(data) < 2 do
    throw({:invalid_tlv, "Insufficient data for TLV header (need 2 bytes, have #{byte_size(data)})"})
  end
    ...
    case decode_length(length_byte, rest) do
      {:ok, length, value_and_rest} ->
        if byte_size(value_and_rest) >= length do
          ...
          {:error, "Insufficient data for TLV value (need #{length} bytes, have #{byte_size(value_and_rest)})"}
        end
      {:error, reason} ->
        {:error, reason}
    end
  ...
  def decode_length(length_byte, remaining_data) do
    cond do
      ...
      length_byte > 0x84 ->
        {:error, "Invalid length encoding 0x#{Integer.to_string(length_byte, 16)} (too many length bytes)"}
  ...
  defp decode_extended_length(num_bytes, data) when num_bytes > byte_size(data) do
    {:error, "Insufficient data for extended length (need #{num_bytes} bytes, have #{byte_size(data)})"}
  end

  # 1-byte extended length (0x81)
  defp decode_extended_length(1, <<length::8, rest::binary>>) do
    if length <= 0x7F do
      Logger.warning("Extended length 0x81 used for short length #{length} (should use direct encoding)")
    end
    {:ok, length, rest}
  end
  ...
  defp decode_extended_length(2, <<length::16, rest::binary>>) do
    if length <= 0xFF do
      Logger.warning("Extended length 0x82 used for length #{length} (could use shorter encoding)")
    end
    {:ok, length, rest}
  end
  ...
  defp decode_extended_length(3, <<length::24, rest::binary>>) do
    if length <= 0xFFFF do
      Logger.warning("Extended length 0x83 used for length #{length} (could use shorter encoding)")
    end
    {:ok, length, rest}
  end
  ...
  defp decode_extended_length(4, <<length::32, rest::binary>>) do
    if length <= 0xFFFFFF do
      Logger.warning("Extended length 0x84 used for length #{length} (could use shorter encoding)")
    end
    
    # Sanity check for unreasonably large lengths
    if length > 100_000_000 do
      Logger.error("Decoded extremely large length: #{length} bytes. This may indicate a parsing error or corrupted data.")
  ...
  def try_alternative_length_decoding(<<b1, b2, b3, b4>>) do
    ...
    alternatives = [
      ...
      data_len, 
      sum_interpretation,
      b4  # Maybe only the last byte matters
    ]
    ...
    case binary do
      <<type::8, 0x84, b1::8, b2::8, b3::8, b4::8, _rest::binary>> ->
        ...
        %{
          ...
          hex_representation: "0x#{Integer.to_string(b1, 16)}#{Integer.to_string(b2, 16)}#{Integer.to_string(b3, 16)}#{Integer.to_string(b4, 16)}"
        }
