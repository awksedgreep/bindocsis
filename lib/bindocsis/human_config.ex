defmodule Bindocsis.HumanConfig do
      ...
      {:ok, enhanced_tlvs} when is_list(enhanced_tlvs) ->
        human_tlvs =
          Enum.map(enhanced_tlvs, fn tlv ->
            formatted_val =
              Map.get(tlv, :formatted_value) || format_raw_value(tlv.value_type, tlv.value)
            ...
            base_tlv =
              if Map.has_key?(tlv, :subtlvs) and is_list(tlv.subtlvs) and length(tlv.subtlvs) > 0 and
                   tlv.value_type != :hex_string do
                # Recursively process subtlvs - they should also be enriched TLVs
                human_subtlvs =
                  Enum.map(tlv.subtlvs, fn subtlv ->
                    sub_formatted_val =
                      Map.get(subtlv, :formatted_value) ||
                        format_raw_value(subtlv.value_type, subtlv.value)
                    ...
                    if Map.has_key?(subtlv, :subtlvs) and is_list(subtlv.subtlvs) and
                         length(subtlv.subtlvs) > 0 do
                      # Recursively process sub-subtlvs to create clean JSON structure
                      clean_sub_subtlvs =
                        Enum.map(subtlv.subtlvs, fn sst ->
                          sst_formatted_val =
                            Map.get(sst, :formatted_value) ||
                              format_raw_value(sst.value_type, sst.value)
    ...
    with {:ok, type} <- extract_tlv_type(human_tlv),
      ...
      if value_type == :hex_string do
        ...
        case Map.get(human_tlv, "subtlvs") do
          ...
          _ ->
            # Convert simple TLV
            # For sub-TLVs with enum values, reverse-lookup the human-readable value to numeric
            with {:ok, human_value} <- extract_human_value(human_tlv),
                 resolved_value = reverse_lookup_enum_value(parent_type, type, human_value),
                 {:ok, binary_value} <- ValueParser.parse_value(value_type, resolved_value) do
              binary_tlv = %{
                type: type,
                length: byte_size(binary_value),
                value: binary_value
  ...
  defp convert_compound_tlv_to_binary(type, subtlvs, docsis_version) do
    ...
    case convert_human_tlvs_to_binary(subtlvs, docsis_version, type) do
      {:ok, binary_subtlvs} ->
        ...
        subtlv_binary =
          Enum.reduce(binary_subtlvs, <<>>, fn subtlv, acc ->
  ...
  # Reverse-lookup enum values: convert "50 kHz" back to 1 for parsing
  # Returns the numeric key if found in enum_values, otherwise returns the original value
  defp reverse_lookup_enum_value(nil, _type, value), do: value

  defp reverse_lookup_enum_value(parent_type, type, value) when is_binary(value) do
    case SubTlvSpecs.get_subtlv_info(parent_type, type) do
      {:ok, %{enum_values: enum_values}} when is_map(enum_values) and map_size(enum_values) > 0 ->
        # Find the key where the enum value matches
        case Enum.find(enum_values, fn {_key, enum_val} -> enum_val == value end) do
          {key, _} -> key
          nil -> value
        end
  ...
  defp reverse_lookup_enum_value(_parent_type, _type, value), do: value
  ...
  defp get_template_description(:docsis30), do: "DOCSIS 3.0 with channel bonding service flows"
  defp get_template_description(:docsis31), do: "DOCSIS 3.1 with OFDM/OFDMA profiles"
  ...
  def get_available_templates do
    [:residential, :business, :minimal, :gigabit, :ipv6, :docsis30, :docsis31]
  end
end
