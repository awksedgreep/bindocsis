defmodule Bindocsis.Validator do
  ...
  # Downstream Frequency, Upstream Channel ID, Network Access Control
  @required_basic_tlvs [1, 2, 3]
  ...
  @tlv_dependencies %{
    # Downstream Service Flow requires frequency and upstream channel
    24 => [1, 2],
    # Upstream Service Flow requires frequency and upstream channel
    25 => [1, 2],
    # Baseline Privacy Config requires Key Management
    30 => [31],
    # SNMPv3 Kickstart may require Subscriber Management Control
    38 => [39]
  }
  ...
  def validate_configuration(tlvs, opts \\ []) when is_list(tlvs) do
    ...
    {tlv_errors, tlv_warnings, tlv_info} =
      validate_individual_tlvs(tlvs, docsis_version, strict_mode)
    ...
    summary = %{
      ...
      valid_tlvs:
        length(tlvs) - length(Enum.filter(all_errors, &(&1.severity in [:critical, :error]))),
  ...
  ## Examples
      ...
      {:ok, []}  # No errors
  """
  @spec validate_tlv(map(), String.t(), boolean()) ::
          {:ok, [validation_error()]} | {:error, String.t()}
  ...
  """
  @spec validate_value(atom(), binary(), keyword()) ::
          {:ok, [validation_error()]} | {:error, String.t()}
  ...
  defp validate_dependencies(tlvs) do
    ...
    errors =
      Enum.flat_map(@tlv_dependencies, fn {tlv_type, required_types} ->
      ...
      if MapSet.size(missing_required) > 0 do
        [
          %{
            ...
            tlv_type: nil,
            message:
              "Missing required TLVs: #{MapSet.to_list(missing_required) |> Enum.join(", ")}",
  ...
  defp validate_length_logic(actual_length, declared_length, tlv_type, strict_mode) do
    errors =
      if actual_length != declared_length do
        [
          create_error(
            :length_mismatch,
            tlv_type,
  ...
  defp validate_tlv_value(tlv, _docsis_version, _strict_mode) do
    case DocsisSpecs.get_tlv_info(tlv.type) do
      ...
      # Unknown TLV, skip value validation
      {:error, _} ->
        []
    end
  end
  ...
  defp create_warning(type, tlv_type, message, recommendation) do
    %{
      ...
      # Add severity for consistent structure
      severity: :warning
    }
  end
end
