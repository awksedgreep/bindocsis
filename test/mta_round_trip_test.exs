defmodule MtaRoundTripTest do
  ...
  describe "MTA Binary Round-Trip" do
    ...
    test "parses and regenerates test_mta.bin byte-perfectly" do
      # KNOWN LIMITATIONS:
      # 1. Precision rounding: Formatted values like "305.42 MHz" (human-readable) lose
      #    precision when round-tripped (305.419896 MHz → 305.42 MHz = 104 Hz difference).
      #    This is expected behavior for human-editable formats.
      # 2. The MTA parser uses heuristics to detect TLV boundaries (specifically for TLV 84
      #    "Line Package" vs 0x84 extended length indicator). This can create zero-length
      #    TLVs that weren't explicitly encoded in the original. The regenerated binary will
      #    have explicit length bytes, causing a 1-byte difference.
      # Both limitations are acceptable since the semantic content is preserved.
    ...
    test "handles MTA files without terminator" do
      ...
      # Type(1) + Len(1) + Val(4) + Type(1) + Len(1) + Val(1) = 9
    ...
    test "handles MTA files with 0xFF 0x00 0x00 terminator" do
      ...
      assert {:ok, binary} =
               Bindocsis.generate(tlvs, format: :mta, terminate: true, terminator: :ff_00_00)
  ...
  describe "Edge Cases" do
    ...
    test "handles multiple TLVs with mixed lengths" do
      tlvs = [
        # Single byte length
        %{type: 1, length: 4, value: <<1, 2, 3, 4>>},
        # 0x81 length
        %{type: 2, length: 200, value: :binary.copy(<<5>>, 200)},
        # 0x82 length
        %{type: 3, length: 1000, value: :binary.copy(<<6>>, 1000)},
        # Empty
        %{type: 4, length: 0, value: <<>>}
      ]
    ...
    test "rejects length mismatch" do
      # Claims 10, has 3
      invalid_tlvs = [%{type: 5, length: 10, value: <<1, 2, 3>>}]
