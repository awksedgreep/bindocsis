defmodule Bindocsis.ComprehensiveHumanInputTest do
  ...
  describe "uint32 human input parsing" do
    test "parses various uint32 formatted values" do
      ...
      assert {:ok, <<255, 255, 255, 255>>} =
               Bindocsis.ValueParser.parse_value(:uint32, "4294967295")
  ...
  describe "frequency human input parsing" do
    test "parses frequency values with units" do
      # MHz formats
      assert {:ok, <<35, 57, 241, 192>>} =
               Bindocsis.ValueParser.parse_value(:frequency, "591 MHz")
      ...
      assert {:ok, <<59, 154, 202, 0>>} =
               Bindocsis.ValueParser.parse_value(:frequency, "1000 MHz")

      # Hz formats
      assert {:ok, <<35, 57, 241, 192>>} =
               Bindocsis.ValueParser.parse_value(:frequency, "591000000 Hz")

      assert {:ok, <<35, 57, 241, 192>>} =
               Bindocsis.ValueParser.parse_value(:frequency, "591000000")

      # Decimal values
      assert {:ok, <<35, 61, 194, 80>>} =
               Bindocsis.ValueParser.parse_value(:frequency, "591.25 MHz")
    end
  ...
  describe "ipv4 human input parsing" do
    test "parses IPv4 addresses" do
      ...
      assert {:ok, <<255, 255, 255, 255>>} =
               Bindocsis.ValueParser.parse_value(:ipv4, "255.255.255.255")
  ...
  describe "ipv6 human input parsing" do
    test "parses IPv6 addresses" do
      # Full format
      {:ok, result} =
        Bindocsis.ValueParser.parse_value(:ipv6, "2001:0db8:85a3:0000:0000:8a2e:0370:7334")
  ...
  describe "binary human input parsing" do
    test "parses hex string values" do
      ...
      assert {:ok, <<0xDE, 0xAD, 0xBE, 0xEF>>} =
               Bindocsis.ValueParser.parse_value(:binary, "DEADBEEF")
    ...
    test "rejects invalid hex formats" do
      ...
      # Odd length
      assert {:error, _} = Bindocsis.ValueParser.parse_value(:binary, "1", strict: true)
  ...
  describe "service_flow_ref human input parsing" do
    test "parses service flow references" do
      case Bindocsis.ValueParser.parse_value(:service_flow_ref, "Service Flow #1") do
        {:ok, _} ->
          assert {:ok, _} =
                   Bindocsis.ValueParser.parse_value(:service_flow_ref, "Service Flow #1")

          assert {:ok, _} =
                   Bindocsis.ValueParser.parse_value(:service_flow_ref, "Service Flow #255")
      ...
      test_config = %{
        ...
        "tlvs" => [
          %{
            ...
            # User edited from 591 MHz
            "formatted_value" => "600 MHz",
            "value_type" => "frequency"
          },
          %{
            "type" => 3,
            "name" => "Network Access Control",
            # User edited from disabled
