defmodule Bindocsis.Integration.FixtureRoundTripSampleTest do
  ...
  describe "Sample fixture round-trip tests" do
    test "sample fixtures maintain data integrity through JSON round-trip" do
      ...
      results =
        Enum.map(sample_fixtures, fn fixture_path ->
      ...
      success_rate =
        if total > 0, do: (length(successes) / total * 100) |> Float.round(1), else: 0.0
        ...
        {json_output, 0} ->
          ...
          case System.cmd(
            ...
            {error_output, exit_code} ->
              {:error,
               {fixture_path,
                "Binary generation failed (#{exit_code}): #{String.trim(error_output)}"}}
          end

        {error_output, exit_code} ->
          {:error,
           {fixture_path, "JSON parsing failed (#{exit_code}): #{String.trim(error_output)}"}}
      end
    rescue
      e ->
  ...
  defp create_vendor_test_config do
    ...
    tlvs = [
      ...
      # OUI + data
      %{type: 202, length: 7, value: <<0x2B, 0x05, 0x08, 0x03, 0x01, 0x02, 0x03>>},
