defmodule Bindocsis.Integration.CLITest do
  ...
  setup_all do
    ...
    unless File.exists?(test_binary) do
      ...
      # Network Access + End marker
      minimal_config = <<3, 1, 1, 255, 0, 0>>
  ...
  describe "main/1 error handling" do
    ...
    test "shows error for nonexistent file" do
      ...
      assert combined_output =~ "Input file does not exist" or
               combined_output =~ "Failed to read file"
    end
  ...
  describe "validate command" do
    ...
    test "validates JSON file", %{test_json: json_file} do
      ...
      assert combined_output =~ "valid" or combined_output =~ "Validation" or
    ...
    test "validates with specific DOCSIS version", %{test_binary: binary_file} do
      ...
      assert combined_output =~ "3.0" or combined_output =~ "valid" or
               combined_output =~ "Validation"
    end
  ...
  describe "error recovery and edge cases" do
    test "handles malformed JSON gracefully" do
      ...
      malformed_json = ~s({"tlvs": [{"type": 3, "value": 1})
    ...
    test "handles invalid hex strings" do
      ...
      assert combined_output =~ "error" or combined_output =~ "hex" or
