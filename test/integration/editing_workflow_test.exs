defmodule Bindocsis.EditingWorkflowTest do
  ...
  This addresses the concern about whether our round-trip tests actually
  validate the editing use case that users depend on.
  """
    ...
    if fixture_file do
      ...
      {full_output, 0} = System.cmd("mix", ["run", "-e", "
        case File.read(\"#{fixture_file}\") do
          {:ok, binary_config} ->
            case Bindocsis.HumanConfig.to_json(binary_config) do
      ...
      json_output = case Regex.run(~r/JSON_START(.*)JSON_END/s, full_output) do
        [_, json] -> json
      ...
      if length(snmp_tlvs) > 0 do
        ...
        {_output, 0} = System.cmd("mix", ["run", "-e", "
          case File.read(\"#{temp_json_file}\") do
            {:ok, json_content} ->
        ...
        {verify_full_output, 0} = System.cmd("mix", ["run", "-e", "
          case File.read(\"#{temp_bin_file}\") do
            {:ok, binary_config} ->
              case Bindocsis.HumanConfig.to_json(binary_config) do
                {:ok, json_result} -> IO.puts(\"JSON_START\#{json_result}JSON_END\")
                {:error, reason} -> IO.puts(\"Error: \#{reason}\"); System.halt(1)
        ...
        verify_json = case Regex.run(~r/JSON_START(.*)JSON_END/s, verify_full_output) do
          [_, json] -> json
  ...
  test "Vendor TLV editing workflow - modify OUI and data" do
    ...
    if fixture_file do
      ...
      if length(vendor_tlvs) > 0 do
        ...
        verify_json = case Regex.run(~r/JSON_START(.*)JSON_END/s, verify_full_output) do
          [_, json] -> json
          nil -> raise "Could not find JSON in vendor verification output: #{verify_full_output}"
        end
        final_data = JSON.decode!(verify_json)
  ...
  defp find_snmp_mib_tlvs_recursive([tlv | rest], acc) when is_map(tlv) do
    new_acc = if Map.get(tlv, "type") == 11 do
      [tlv | acc]
    else
  ...
  defp find_vendor_tlvs_recursive([tlv | rest], acc) when is_map(tlv) do
    ...
    new_acc = if is_integer(type) and type >= 200 and type <= 254 do
      [tlv | acc]
    else
      acc
    end
      ...
      [first_snmp | _] ->
        ...
        case original_formatted do
          %{"oid" => original_oid, "type" => type, "value" => original_value} ->
            ...
            Logger.info("Modifying SNMP MIB: OID #{original_oid} -> #{modified_oid}, Value #{inspect(original_value)} -> #{inspect(modified_value)}")
          ...
          _ ->
            Logger.warn("SNMP TLV doesn't have expected structured format: #{inspect(original_formatted)}")
            data
        end
      [] ->
        data
      ...
      [first_vendor | _] ->
        ...
        case original_formatted do
          %{"oui" => original_oui, "data" => original_data} ->
            ...
            Logger.info("Modifying Vendor TLV: OUI #{original_oui}, Data #{original_data} -> #{modified_data}")
          ...
          _ ->
            Logger.warn("Vendor TLV doesn't have expected structured format: #{inspect(original_formatted)}")
            data
        end
      [] ->
        data
  ...
  defp update_tlvs_in_list([tlv | rest], target_type, new_formatted_value) when is_map(tlv) do
    updated_tlv = if Map.get(tlv, "type") == target_type do
  ...
  defp verify_snmp_modifications(original_data, final_data, _snmp_tlvs) do
    ...
    changes_found = Enum.zip(original_snmp, final_snmp)
  ...
  defp verify_vendor_modifications(original_data, final_data, _vendor_tlvs) do
    ...
    changes_found = Enum.zip(original_vendor, final_vendor)
    |> Enum.any?(fn {orig, final} ->
      orig_formatted = Map.get(orig, "formatted_value", %{})
      final_formatted = Map.get(final, "formatted_value", %{})
      orig_formatted != final_formatted
  ...
  defp test_synthetic_snmp_editing() do
    ...
    test_tlvs = [
      ...
      %{type: 255, value: <<>>, length: 0}  # End marker
    ]
  ...
  defp test_synthetic_vendor_editing() do
    ...
    vendor_oui = <<0x00, 0x10, 0x95>>  # Broadcom
  ...
  defp test_editing_workflow_on_file(file_path, test_type) do
    ...
    modified_data = case test_type do
      :snmp ->
        snmp_tlvs = find_snmp_mib_tlvs(original_data)
    ...
    final_json = case Regex.run(~r/JSON_START(.*)JSON_END/s, final_full_output) do
      [_, json] -> json
