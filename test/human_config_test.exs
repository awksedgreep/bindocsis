defmodule Bindocsis.HumanConfigTest do
  ...
  describe "binary to human conversion" do
    ...
    test "includes descriptions when requested" do
      assert {:ok, json_string} =
               HumanConfig.to_json(@sample_binary_config, include_descriptions: true)
    ...
    test "handles different format styles" do
      ...
      assert {:ok, compact_json} =
               HumanConfig.to_json(@sample_binary_config, format_style: :compact)

      assert {:ok, compact_parsed} = JSON.decode(compact_json)

      # Verbose format
      assert {:ok, verbose_json} =
               HumanConfig.to_json(@sample_binary_config, format_style: :verbose)
  ...
  describe "human to binary conversion" do
    test "converts YAML config to binary successfully" do
      ...
      # 591 MHz in binary
      assert frequency_tlv.value == <<35, 57, 241, 192>>
    ...
    test "validates configuration by default" do
      ...
      tlvs:
        - type: 1
          formatted_value: "invalid frequency"
      """
  ...
  describe "round-trip validation" do
    ...
    test "detects round-trip inconsistencies" do
      ...
      # Unknown TLV type
      problematic_binary = <<200, 5, "hello", 255>>
  ...
  describe "template generation" do
    ...
    test "generates business template" do
      ...
      assert String.contains?(yaml_template, "615 MHz")
      # Business IP range
      assert String.contains?(yaml_template, "10.1.1.100")
      # Higher CPE limit
      assert String.contains?(yaml_template, "64")
    end
    ...
    test "generates minimal template" do
      ...
      # Only essential TLVs
      assert length(tlv_lines) == 2
    end

    test "includes descriptions in templates when requested" do
      assert {:ok, yaml_template} =
               HumanConfig.generate_template(:residential, include_descriptions: true)
  ...
  describe "error handling" do
    ...
    test "handles invalid JSON gracefully" do
      invalid_json = """
      {
        ...
        "tlvs": [
          {
            "type": 1,
            "formatted_value": "591 MHz"  // Comments not allowed in JSON
          }
        ]
      }
      """
    ...
    test "handles unparseable values" do
      ...
      tlvs:
        - type: 1
          formatted_value: "not a valid frequency"
      """
  ...
  describe "integration tests" do
    test "full workflow: binary -> YAML -> binary" do
      ...
      assert {:ok, original_tlvs} =
               Bindocsis.parse(original_binary, format: :binary, enhanced: false)

      assert {:ok, reconverted_tlvs} =
               Bindocsis.parse(reconverted_binary, format: :binary, enhanced: false)
