defmodule Bindocsis.ConfigComparatorTest do
  ...
  describe "compare/3" do
    test "compares identical configurations" do
      ...
      changes_without_unchanged = Enum.filter(comparison.tlv_changes, &(&1.change_type != :unchanged))
    ...
    test "detects frequency change as critical modification" do
      assert {:ok, comparison} = ConfigComparator.compare(@config_original, @config_modified_frequency)
      ...
      assert String.contains?(frequency_change.description, "615") and String.contains?(frequency_change.description, "MHz")
    end

    test "detects additions and modifications" do
      assert {:ok, comparison} = ConfigComparator.compare(@config_original, @config_with_additions)
    ...
    test "detects removals" do
      ...
      assert length(removals) == 3  # Upstream channel, IP, CPE limit
      
      # Check that removed TLVs have appropriate impact levels
      removed_types = Enum.map(removals, & &1.tlv_type)
      assert 2 in removed_types  # Upstream channel
      assert 12 in removed_types  # IP address
      assert 21 in removed_types  # CPE limit
    end
    ...
    test "generates change statistics correctly" do
      ...
      assert stats.high_impact_count >= 1  # Service flow addition
      assert stats.medium_impact_count >= 1  # CPE modification
    end
    ...
    test "respects comparison options" do
      ...
      assert {:ok, comparison} = ConfigComparator.compare(@config_original, @config_original, opts)
    ...
    test "handles invalid configurations gracefully" do
      invalid_config = <<1, 255, 2>>  # Invalid length
  ...
  describe "generate_diff_report/1" do
    ...
    test "handles comparison with additions and removals" do
      ...
      assert String.contains?(report, "TLV 2")  # Upstream channel
      assert String.contains?(report, "TLV 12")  # IP address
      assert String.contains?(report, "TLV 21")  # CPE limit
    end
  ...
  describe "analyze_impact/1" do
    ...
    test "returns error when impact analysis not performed" do
      assert {:ok, comparison} = ConfigComparator.compare(@config_original, @config_original, include_impact_analysis: false)
  ...
  describe "are_compatible?/1" do
    ...
    test "handles comparison without compatibility check" do
      assert {:ok, comparison} = ConfigComparator.compare(@config_original, @config_original, check_compatibility: false)
      assert ConfigComparator.are_compatible?(comparison) == false  # No assessment performed
    end
  end
  ...
  describe "get_change_statistics/1" do
    test "returns change statistics" do
      ...
      assert stats.added_count == 1  # Service flow
      assert stats.modified_count == 1  # CPE limit
      assert stats.removed_count == 0
    end
  ...
  describe "service disruption risk assessment" do
    test "identifies high disruption risk for critical removals" do
      ...
      config_no_frequency = <<3, 1, 1, 255>>  # Only network access
  ...
  describe "change recommendations" do
    test "provides appropriate recommendations for critical changes" do
      ...
      critical_recommendation = Enum.find(recommendations, fn rec ->
    ...
    test "provides frequency-specific recommendations" do
      ...
      frequency_recommendation = Enum.find(recommendations, fn rec ->
